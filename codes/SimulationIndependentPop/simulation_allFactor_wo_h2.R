...
BVs = matrix(0, nrow = nInd, ncol = nFactors)
BVs[,1] = sapply(1:effective_factor, function(x) X2[, QTL_pos[[x]]] %*% QTL_effects[[x]])
colnames(BVs) = paste0("factor", 1:K)
...
BV_var = var(BVs[,"factor1"])


### Simulate E_F
vare_f = (1-h2_F)/h2_F * BV_var
EF_1 = rnorm(nInd) * sqrt(vare_f)
var_F1 = var(BVs[,1] + EF_1)
EF = sapply(1:K, function(x) rnorm(nInd, sd = sqrt(var_F1)))
EF[,1] = EF_1
### Simulate F 
F_matrix = BVs +  EF

print(sprintf("The simulated h2F for factor %d is %f",1, var(BVs[,1])/var(F_matrix[,1])))
apply(F_matrix,2,var)
