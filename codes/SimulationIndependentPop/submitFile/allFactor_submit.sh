...
Rcode="$currentpath/Codes/simulation_allFactor.R"
...
do
	...
	do
		...
		do
			echo "snp number = $j"
			simulation_path="$currentpath/allFactor/${i}factor_${nTraits}trait_${j}snp_0.95h2F"
			mkdir $simulation_path
			for r in {1..10}
			do
				echo "repeat $r"
				echo "#SBATCH -J snp${j}_rep$r" > job_name
				echo "#SBATCH -D $currentpath/allFactor/" > path_name
				echo module load R >load_module
				echo Rscript $Rcode $simulation_path $code_path $nTraits $i $j $r  > ender
				file="allFactor_trait${nTraits}_factor${i}_snp${j}_rep$r.sh"
