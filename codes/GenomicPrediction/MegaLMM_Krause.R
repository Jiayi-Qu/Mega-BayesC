...
runID = sprintf('MegaLMM_GBLUP_%d',foldid)
...
U = get_posterior_mean(MegaLMM_state,U_R[,1,drop=F] + U_F %*% Lambda[,1,drop=FALSE],bychunk = T)

MegaLMM_state$Posterior$Lambda = load_posterior_param(MegaLMM_state,'Lambda')
MegaLMM_state$Posterior$U_F = load_posterior_param(MegaLMM_state,'U_F')
MegaLMM_state$Posterior$U_R = load_posterior_param(MegaLMM_state,'U_R')

write.csv(get_posterior_mean(MegaLMM_state, Lambda), sprintf('%s/Posterior/Posterior_Lambda.csv',runID))
write.csv(get_posterior_mean(MegaLMM_state, U_F), sprintf('%s/Posterior/Posterior_U_F.csv',runID))
write.csv(get_posterior_mean(MegaLMM_state, U_R), sprintf('%s/Posterior/Posterior_U_R.csv',runID))

print("csv saved.")

results = data.frame(Method = 'MegaLMM_GBLUP',
...
results$fold = foldid
write.csv(results,file = sprintf('%s/results_MegaLMM_GBLUP_fold_%d.csv',results_dir,foldid))
