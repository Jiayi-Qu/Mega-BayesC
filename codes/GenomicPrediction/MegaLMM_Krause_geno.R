...
runID = sprintf('MegaLMM_Krause_Horseshoe_%d',foldid)
...
MegaLMM_state$Posterior$posteriorSample_params = c('Lambda','F_h2','B2_F')
MegaLMM_state = clear_Posterior(MegaLMM_state)
...
for(i  in 1:100) {
  ...
  # U = get_posterior_FUN(MegaLMM_state,X2_F %*% B2_F %*% t(Lambda[1:2,]))
  # U = U[,nas,1]
...
U = get_posterior_mean(MegaLMM_state,X2_F %*% B2_F %*% t(Lambda[1:2,]))
...
results = data.frame(Method = 'MegaLMM_Horseshoe',
...
results$fold = foldid
write.csv(results,file = sprintf('%s/results_MegaLMM_Horseshoe_fold_%d.csv',results_dir,foldid))
